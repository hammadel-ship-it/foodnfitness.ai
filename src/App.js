    ...
    <Modal onClose={onClose}>
      ...
      <div style={{marginBottom:20}}>
        <div style={{display:"flex",gap:12,marginBottom:20}}>
          <div style={{flex:1}}>
            <div style={{color:"#4a7a56",fontSize:".78rem",letterSpacing:".1em",textTransform:"uppercase",marginBottom:8}}>Age</div>
            <input type="number" min="10" max="100" value={age} onChange={e=>setAge(e.target.value)} placeholder="e.g. 34"
              style={{width:"100%",background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.12)",borderRadius:10,padding:"10px 12px",color:"#d8f0de",fontSize:".95rem",fontFamily:"Georgia,serif",boxSizing:"border-box",outline:"none"}}/>
          </div>
          <div style={{flex:1}}>
            <div style={{color:"#4a7a56",fontSize:".78rem",letterSpacing:".1em",textTransform:"uppercase",marginBottom:8}}>Weight <span style={{textTransform:"none",letterSpacing:0,fontSize:".72rem"}}>(kg)</span></div>
...
function WeekPlan({ plan }) {
  ...
  return(
    <div style={{marginTop:28}}>
      <div style={{color:"#6aaa80",fontSize:".85rem",letterSpacing:".1em",textTransform:"uppercase",marginBottom:6}}> Your 7-day wellness plan</div>
        ...
        return(
          <div key={i} className="rc" style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(34,163,90,.18)",borderRadius:14,overflow:"hidden",marginBottom:10,transition:"border-color .18s"}}>
            <button onClick={()=>setActiveRecipe(open?null:rid)} style={{width:"100%",textAlign:"left",background:"transparent",border:"none",padding:"16px 18px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span style={{color:"#c8e8ce",fontSize:"clamp(1rem,1.6vw,1.12rem)",fontWeight:600}}>{r.emoji||""} {r.name}</span>
    ...
    <div style={{minHeight:"100vh",background:"#0b1a0d",color:"#e0ede2",fontFamily:"'Georgia',serif"}}>
      <div style={{position:"relative",zIndex:1,maxWidth:1060,margin:"0 auto",padding:"0 22px 90px"}}>
        ...
        <div style={{textAlign:"center",marginTop:42,display:"flex",justifyContent:"center",gap:24,flexWrap:"wrap"}}>
          {[" Secure via Stripe"," Cancel anytime"," Food   Fitness   Breath   Sleep"].map((s,i)=><span key={i} style={{color:"#1e3d25",fontSize:".86rem"}}>{s}</span>)}
        </div>
        ...
        <div style={{display: hasConvo ? "none" : "block"}}>
            <div style={{textAlign:"center",padding:"clamp(36px,6vw,72px) clamp(24px,8vw,120px) clamp(20px,3vw,36px)"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:8,marginTop:6,marginBottom:4}}>
                ...
                {user?.sex&&<div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(34,163,90,.07)",border:"1px solid rgba(34,163,90,.18)",borderRadius:20,padding:"3px 12px"}}><span style={{color:"#4a7a56",fontSize:".82rem"}}>{user.sex==="female"?" Female profile":" Male profile"}</span></div>}
                {user&&(!user.sex||!user.age||!user.weight)&&<button onClick={()=>setShowProfile(true)} style={{background:"none",border:"1px dashed rgba(34,163,90,.25)",borderRadius:20,padding:"3px 12px",color:"#2a4a30",fontSize:".78rem",cursor:"pointer",fontFamily:"'Georgia',serif",fontStyle:"italic"}}>
